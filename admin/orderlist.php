...
<body> 
       <div class="main">
		...
		<div class="slider" align="center">
					...
					<div>
						<table border="1">
							...
							<?php
								...
								$query=mysqli_query($con,"SELECT * FROM `order` inner JOIN package ON `order`.package_id=package.package_id inner JOIN registration ON `order`.username=registration.username WHERE `order`.order_date='".$currentdate."'");
								if($query === FALSE) { 
					...
					<h2>Order Summary</h2>
						<?php 
							$query1=mysqli_query($con,"SELECT package.package_name,sum(quantity) as quantity FROM `order` inner JOIN package ON `order`.package_id=package.package_id WHERE `order`.order_date='".$currentdate."' GROUP BY `order`.package_id");
						...
						?>
						<?php 
							$query2=mysqli_query($con,"SELECT sum(quantity) as quantity FROM `order` WHERE `order`.order_date='".$currentdate."'");
